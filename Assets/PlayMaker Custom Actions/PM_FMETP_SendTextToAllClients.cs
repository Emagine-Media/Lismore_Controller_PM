...
namespace Net.PlayMaker.FMETP
{
    ...
    [HutongGames.PlayMaker.Tooltip("Send a text message from the server to all connected clients. If called on a client, it broadcasts to all (per FMETP behavior).")]
    public class PM_FMETP_SendTextToAllClients : FsmStateAction
    {
        ...
        bool TryCache()
        {
            ...
            if (go == null)
            {
                Debug.LogWarning("[PM_FMETP_SendTextToAllClients] fmNetworkObject is null.");
                return false;
            }
            _fm = go.GetComponent<FMNetworkManager>();
            if (_fm == null)
            {
                Debug.LogWarning("[PM_FMETP_SendTextToAllClients] FMNetworkManager not found on GameObject.");
                return false;
            }
            return true;
        }
        ...
        {
            if (_fm == null) return;
            _fm.SendToAll(message.Value ?? string.Empty);
        }
    }
}
